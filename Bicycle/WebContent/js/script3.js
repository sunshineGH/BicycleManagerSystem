...
app.config(function($routeProvider) {
	...
	.when('/wageInfo',{
		controller:'wageInfoController',
		templateUrl:'pages/wageInfo.html'
	})
	.when('/pswChange',{
...
app.controller('userController',function($scope,$http,$location,$cookieStore) {
	...
	var getUserList = function() {
		$http({
			...
			processData:false,
			cache:false,
			params:{method:'getUserList',currentpage:$scope.paginationConf.currentPage,isadmin:0}
	...
	$scope.deleteUser = function(worker) {
		...
		layer.confirm('确定删除该记录吗？',{
			...
			$http({
				...
				cache:false,
				params:{id:worker.id,
					level:worker.level,method:'deleteUser'}
			}).success(function(data){
				if(data.result){
...
app.controller('detailController',function($scope,$routeParams,$http,$location,$cookieStore) {
	...
	if($routeParams.id!=null){
		...
		$scope.isShowPsw = false;
		$scope.isLevelSelected = false;
		$http({
			url:'http://10.103.241.137:8080/Beauty_Salon/InfoCl',
			dataType:"jsonp",
			method:'GET',
		...
		$scope.isShowPsw = true;
		$scope.isReadOnly = false;
		$scope.moneyIsReadOnly = true;
		$scope.isLevelSelected = true;
			...
			if($scope.productForm.$valid){
				$http({
					...
					params:{method:'updateProduct',id:$scope.product.id,name:$scope.product.name,
						...
						console.log('success');
						if(!data.result) {
							layer.alert("产品名已存在，请重新输入",{icon:5});
						}else {
							layer.alert("提交成功,信息已修改",{icon:6});
...
app.controller('informationController',function($scope,$http,$cookieStore){
	userid = $cookieStore.get('userid');
	userlevel = $cookieStore.get('userlevel');
	$http({
		url:'http://10.103.241.137:8080/Beauty_Salon/InfoCl',
		dataType:"jsonp",
		method:'GET',
		...
		processData:false,
		cache:false,
		params:{method:'getUser',level:userlevel,id:userid}
	}).success(function(data){
		console.log('success');
		$scope.user = data;
	}).error(function(){
...
app.controller('wageInfoController',function($scope,$http,$cookieStore){
	userid = $cookieStore.get('userid');
	userlevel = $cookieStore.get('userlevel');
	$scope.user = {
		id: userid,
	...
	if(userlevel=='3') {
		baseOrderIndex = 0.05; 
		baseSeviceIndex = 0.07;
		$scope.user.levelName = "美容师";
	}else if(userlevel=='2'){
		baseOrderIndex = 0.09;
		baseSeviceIndex = 0;
		$scope.user.levelName = "店长";
	}else{
		baseOrderIndex = 0.03;
		baseSeviceIndex = 0;
		$scope.user.levelName = "前台";
	}
	$scope.parseInt = parseInt;
	$http({
		url:'http://10.103.241.137:8080/Beauty_Salon/InfoCl',
		...
		processData:false,
		cache:false,
		params:{method:'getSalary',id:userid}
	}).success(function(data){
		console.log('success');
		$scope.salary = data.result;
		$scope.salary.all_salary = parseInt($scope.salary.base_salary) + parseInt($scope.salary.order_salary) * baseOrderIndex
	+ parseInt($scope.salary.service_salary) * baseSeviceIndex;
	}).error(function(){
		console.log('error!');				
	});
});

app.controller('passwordChangeController',function($scope,$http,$cookieStore) {
	id = $cookieStore.get('userid');
	level = $cookieStore.get('userlevel');
	password = $cookieStore.get('userpsw');
